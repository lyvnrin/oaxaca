                    ...
                    {menuOpen && (
                        <>
                            ...
                            <div className="menu-popup">
                                <button className="menu-popup-item" onClick={() => { setMenuOpen(false); onContactWaiter(); }}>
        ...
        <div className={`menu-item-card ${dimmed ? "menu-item-card--dimmed" : ""} ${unavailable ? "menu-item-card--unavailable" : ""} ${lowStock ? "menu-item-card--unavailable" : ""}`}>
            ...
            <div className="card-body">
                ...
                <div className="card-footer">
                    ...
                    {item.allergens.length > 0 && <p className="card-allergens">Contains: {item.allergens.join(", ")}</p>}
                    <span className="card-calories">{item.calories}</span>
                    {lowStock && !unavailable && <p style={{ fontSize: 10, color: "#c0392b", fontWeight: 700, marginTop: 4 }}>⚠ Low stock</p>}
                </div>
                ...
                <div className="customization-content">
                    {sent ? (
                        ...
                        <>
                            ...
                            <div className="customization-footer">
                                <button className="cancel-btn" onClick={onClose}>Cancel</button>
                                <button className="add-to-order-final-btn" onClick={handleSend}>Send to Waiter</button>
                            </div>
                        </>
                    )}
                </div>
                                        ...
                                        <div key={key} className="modal-item">
                                            ...
                                            {customizations.length > 0 && (
                                                <div className="modal-item-customizations">
                                                    {customizations.map((custom, idx) => <p key={idx} className="customization-text">{custom}</p>)}
                                                </div>
                                            )}
...
export default function App() {
    ...
    async function handlePlaceOrder() {
        const items = Object.values(cart).map(({ item, qty }) => ({
            ...
            extras: item.customization?.selectedExtras?.map(e => ({ name: e.name, price: e.price })) || [],
    ...
    return (
        <div className="app">
                    ...
                    {Object.entries(MENU_DATA).map(([sectionName, items]) => (
                        <MenuSection
                            ...
                            items={items.map(item => ({
                                ...item,
                                price: livePrices[item.id] ? `£${livePrices[item.id].toFixed(2)}` : item.price
                            }))}
            ...
            {cartOpen && <CartModal cart={cart} onClose={() => setCartOpen(false)} onUpdateQty={handleUpdateQty} onRemove={handleRemove} onPlaceOrder={handlePlaceOrder} />}
